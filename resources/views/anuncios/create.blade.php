...
<div class="max-w-3xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold tracking-tight">Novo anúncio</h1>
        <p class="text-sm text-green-300">Crie um anúncio e selecione quais produtos dele fazem parte.</p>
    </div>

    <form method="POST" action="{{ route('anuncios.store') }}" class="bg-zinc-900/70 backdrop-blur border border-green-800 rounded-2xl shadow-sm p-6 space-y-5">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="descricao" rows="4"
                          class="w-full border border-green-800 rounded-xl p-3 bg-zinc-900 focus:outline-none focus:ring-2 focus:ring-slate-300"
                          placeholder="Descreva o anúncio...">{{ old('descricao') }}</textarea>
            </div>

            <div class="sm:col-span-2">
                <label class="block text-sm font-semibold text-green-300 mb-1">Preço de venda</label>
                <input name="preco_venda" value="{{ old('preco_venda') }}"
                       class="w-full border border-green-800 rounded-xl p-3 bg-zinc-900 focus:outline-none focus:ring-2 focus:ring-slate-300"
            ...
            <div class="sm:col-span-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    @forelse($produtos as $produto)
                        @php
                            $checked = in_array($produto->id, old('produtos', []));
                            $qtdOld = old('quantidades.' . $produto->id, 1);
                        @endphp
